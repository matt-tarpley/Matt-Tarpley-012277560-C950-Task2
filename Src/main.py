...
def loadPackageData(fileName, hashTable):
    with open(fileName) as packageFile:
        ...
        for package in packageData:
            ...
            weight = package[6]
            if len(package) > 7 and package[7]: #check for notes
                notes = package[7]
            else:
                notes = None    
            status = PackageStatus.AT_HUB

            packageFromFile = Package(id, address, city, state, zip, deliveryDeadline, weight, notes, status)
...
# truck1 = Truck.Truck(None, [1, 13, 14, 15, 16, 20, 29, 30, 31, 34, 37, 40], 0.0, "4001 South 700 East",
#                      datetime.timedelta(hours=8))

# # Create truck object truck2
# truck2 = Truck.Truck(None, [3, 6, 12, 17, 18, 19, 21, 22, 23, 24, 26, 27, 35, 36, 38, 39], 0.0,
#                      "4001 South 700 East", datetime.timedelta(hours=10, minutes=20))

# # Create truck object truck3
# truck3 = Truck.Truck(None, [2, 4, 5, 6, 7, 8, 9, 10, 11, 25, 28, 32, 33], 0.0, "4001 South 700 East",
...
#         truck.currentTime += datetime.timedelta(hours=next_address / 18)
#         next_package.timeDelivered = truck.currentTime
#         next_package.timeOnTruck = truck.departTime
...
# truck3.departTime = min(truck1.currentTime, truck2.currentTime)
